      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-brand-500/10 border border-brand-500/20 mb-4">
            <span className="text-xs font-semibold text-brand-400 uppercase tracking-widest">Leadership</span>
          </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {team.map((member) => (
            <div key={member.name} className="group p-6 rounded-2xl glass hover:bg-white/[0.06] transition-all duration-300">
              <div className="flex items-start gap-4 mb-4">
                <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${member.color} flex items-center justify-center flex-shrink-0`}>
                  <span className="font-display font-bold text-white text-base">{member.initials}</span>
                </div>
                <div>
                  <h3 className="font-display font-semibold text-white text-base leading-snug">{member.name}</h3>
                  <p className="text-brand-400 text-xs font-medium mt-0.5">{member.role}</p>
                </div>
              </div>
              <p className="text-white/45 text-sm leading-relaxed mb-4">{member.bio}</p>
              <a
